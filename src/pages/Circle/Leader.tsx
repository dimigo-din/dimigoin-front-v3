...
import {
    Button, Card, CardGroupHeader, Checkbox, Col, Data, Divider, HeadData, HeadRow,
    NoData, PageWrapper, ResponsiveWrapper, Row, showModal, Table as _Table
} from "../../components"
import { circleApplicationStatusKorMapper, CircleApplicationStatusLevelTree, CircleApplicationStatusValues } from "../../constants"
import { CircleApplication, CirclePeriod, Doc, Merge, Student } from "../../constants/types"
...
const isChangable = (currentPeriod: keyof typeof CirclePeriod, applicationStatus: typeof CircleApplicationStatusValues[number]) => {
    return ({ application: 1, interview: 3, final: 0 })[currentPeriod] > CircleApplicationStatusValues.indexOf(applicationStatus)
}

const ApplicationList: React.FC<{
    applications?: Doc<ApplicationForSubmiter>[] | null;
    changeState(submitType: SubmitType, selectedApplications: Doc<ApplicationForSubmiter>[]): void;
    openDetail(index: number): void;
    currentPeriod?: string
}> = ({ applications, changeState, openDetail, currentPeriod }) => {
    const [checks, setChecks] = useState<boolean[]>(Array(applications?.length).fill(false))
    ...
    const submit = useCallback((type: SubmitType) => {
        ...
        changeState(type,
            checks.map((checks, index) =>
                checks && applications[index]).filter((e): e is Doc<ApplicationForSubmiter> => !!e)
        )
    ...
    return (<TableWrapper>
                    ...
                    applications
                        ? applications.length ?
                            ...
                            >
                                <Data>
                                    <Checkbox
                                        disabled={currentPeriod ? !isChangable(currentPeriod as keyof typeof CirclePeriod, application.status) : undefined}
                                        checked={checks[index]}
                                        onChange={({ target: { checked } }) => setChecks(beforeState => [
                                ...
                                <Data>{circleApplicationStatusKorMapper[application.status]}</Data>
                        ...
                        : applications === null ? <Data colSpan={7}>
        ...
        <StateChangerWrapper visible={buttonVisible}>
            <Button onClick={() => submit("PASS")} text>불합격</Button>
...
export const Leader: React.FC = () => {
    const [applications, setApplications] = useState<Doc<ApplicationForSubmiter>[] | null>()
    ...
    const changeState = useCallback(async (submitType: SubmitType, selectedApplications: Doc<ApplicationForSubmiter>[]) => {
        ...
        const { isConfirmed } = await swal({
            title: "상태를 변경하시겠어요?",
            html: <>
                <p>{selectedApplications.map(e => e.applier.name).join(', ')} 지원자의 상태를 {circleApplicationStatusKorMapper[changeTo].으로} 변경해요.</p>
                <p>이 작업은 취소할 수 없어요.</p>
            </>,
            imageUrl: DangerIcon,
        ...
        const res = await Promise.allSettled(selectedApplications.map(application =>
            setApplicationStatus(application._id, changeTo)
        ))

        const { fulfilled, rejected } = res.reduce<{
            fulfilled: PromiseFulfilledResult<Doc<Merge<CircleApplication, {
                circle: string;
                applier: Student;
                index: number;
            }>>>[];
            ...
            ...matched,
            [current.status]: [...(matched[current.status] || []), {
                ...current,
                index
            }]
        }), { fulfilled: [], rejected: [] })
