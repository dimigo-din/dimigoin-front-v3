...
const Content: React.FC<Doc<Circle> & { close(): void }> = ({
  _id,
  name,
  close,
}) => {
  const [questions, setQuestions] = useState<
    Doc<CircleApplyQuestionItem>[] | null
  >();
  ...
  const submit = useCallback(() => {
    ...
    const checks = Object.values(answers)
      .map((answer, index) => !answer && index + 1)
      .filter((isWrong) => typeof isWrong !== 'boolean');
    if (checks.length) {
  ...
  return (
    <ContentWrapper>
      <FormWrapper>
        {questions &&
          questions.map(({ question, maxLength, _id }, index) => (
