...
const Outgo: React.FC = () => {
  ...
  const { setValue: setAppliers, ...applierInput } = useInput<
    Doc<BriefStudent>[]
  >();
  ...
  const isTimeSelected = Boolean(
    dateSelectorValue?.[0] && +dateSelectorValue[0],
  );
  const isDateRange =
    dateSelectorValue?.[0].getDate() !== dateSelectorValue?.[1].getDate();
  ...
  const submitHandler = useCallback(() => {
    const alerts = [
      ...
      isTimeSelected &&
        +(dateSelectorValue?.[0] || 0) >= +(dateSelectorValue?.[1] || 0) &&
        '시간 범위',
    ...
    requestOutgo(outgoRequestForm).then(() =>
      toast.success('외출 신청을 완료했습니다!'),
    );
  }, [
    applierValue,
    applyFormValue,
      ...
      <PageWrapper>
        <ResponsiveWrapper threshold={1200}>
          ...
          <Col width={7}>
            ...
            {applyFormValue?.outgoType === 'group' ? (
              ...
              >
                <p>
                  현재 외출 유형이 개인 외출로 지정되어있습니다. 신청자를
                  추가하려면 유형을 단체 외출로 변경해주세요.
                </p>
              </TextCard>
            )}
                  ...
                  {isTimeSelected ? (
                    <Info>
                      {isDateRange ? (
                        <>
                          <DateHighlight>
                            {dateSelectorValue!![0].getMonth() + 1}월{' '}
                            {dateSelectorValue!![0].getDate()}일{' '}
                          </DateHighlight>
                          {dateSelectorValue!![0].getHours()}시{' '}
                          {dateSelectorValue!![0].getMinutes()}분 부터 <br />
                          <DateHighlight>
                            {dateSelectorValue!![1].getMonth() + 1}월{' '}
                            {dateSelectorValue!![1].getDate()}일{' '}
                          </DateHighlight>
                          {dateSelectorValue!![1].getHours()}시{' '}
                          {dateSelectorValue!![1].getMinutes()}분 까지{' '}
                        </>
                      ) : (
                        <>
                          {dateSelectorValue!![0].getMonth() + 1}월{' '}
                          ...
                          <DateHighlight>
                            {dateSelectorValue!![0].getHours()}시{' '}
                            {dateSelectorValue!![0].getMinutes()}분
                          </DateHighlight>{' '}
                          부터{' '}
                          <DateHighlight>
                            {dateSelectorValue!![1].getHours()}시{' '}
                            {dateSelectorValue!![1].getMinutes()}분
                          </DateHighlight>{' '}
                          까지
                        </>
                      )}
