...
const toDropdownList = (items: Doc<Place>[]) =>
  items.map((e) => ({
    name: e.name,
    key: e._id,
  }));
...
export const OtherPlaceModal: React.FC<{
  ...
  useEffect(() => {
    (async () => {
      ...
      if (showOnly)
        setPlaces(() =>
          toDropdownList(placeList.filter((place) => place.type === showOnly)),
        );
      else
        setPlaces(() =>
          toDropdownList(
            priority ? orderTypeFirst(placeList, priority) : placeList,
          ),
        );
    })();
  ...
  const submit = useCallback(() => {
    ...
    if (
      placeDropdown.value?.name &&
      placeDropdown.value.key &&
      reasonInput.value
    )
      onSubmit(
        placeDropdown.value.name,
  ...
  return (
    <CardWrapper>
      <MarginWrapper>
        ...
        <Caution>
          ※ 사전 허가된 활동 또는 감독 교사 승인 외 임의로 등록할 경우 불이익을
          받을 수 있습니다
        </Caution>
      </MarginWrapper>
      <SubmitButton
        active={
          !!(
            placeDropdown.value?.key &&
            reasonInput.value &&
            placeDropdown.value.key !== 'SELECTONE'
          )
        }
        onClick={submit}
      >
