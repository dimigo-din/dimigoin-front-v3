...
const ApplierListCard: React.FC<{
  ...
  return (
    ...
    >
      ...
      <SectionsWrapper>
        {students?.NSS1.length === 0 && students.NSS2.length === 0 ? (
          <NoApplier>신청 인원이 없습니다</NoApplier>
        ) : (
          <>
            <Section>
...
export const IngangsilManager: React.FC = () => {
  const [
    ticketsByGrade,
    setTicketsByGrade,
  ] = useState<TicketsGroupedByGradeAndTime>();
  ...
  useEffect(() => {
    (async () => {
      ...
      const groupedTicket = entireTickets.reduce<TicketsGroupedByGradeAndTime>(
        (grouped, current) => {
          return {
            ...grouped,
            [current.applier.grade]: {
              ...
              [current.time]: [
                ...grouped[current.applier.grade][current.time],
                current.applier,
              ],
            },
          };
