...
export const NoticeEditingModal: React.FC<
  { closeModal(): void } & Partial<Doc<Notice>>
> = ({ closeModal, ...originNotice }) => {
  ...
  const { element: TinyDateRangeSelector, dates } = useTinyDateRangeSelector(
    originNotice.startDate &&
      originNotice.endDate && [originNotice.startDate, originNotice.endDate],
  );
  ...
  const submit = useCallback(() => {
    const alerts = [
      !titleValue && '제목',
      !contentValue && '내용',
      ![grade1Checkbox, grade2Checkbox, grade3Checkbox].some(
    ...
    if (originNotice._id) {
      editRegisteredNotice(originNotice._id, data)
        .then(() => {
  ...
  return (
    <NoticeModalWrapper>
      <HeaderWrapper>
        <TitleInput
          placeholder="ㅁㄴㅇㄹ을 눌러 제목을 입력하세요"
          {...titleInput}
        />
      ...
      <BottomControls threshold={500}>
        <FormWrapper>
          <FormRow>
            ...
            <CheckboxesWrapper>{TinyDateRangeSelector}</CheckboxesWrapper>
          </FormRow>
          <FormRow>
