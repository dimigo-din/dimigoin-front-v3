...
const isRealData = (d: DisplayPlace): d is DisplayPlaceWithStudents =>
  (d as any).students;
...
const SelfStudyDisplay: React.FC<RouteComponentProps> = ({ history }) => {
  ...
  ] = useState<SelfStudyTime | null>(getSelfStudyPeriod());
  ...
  useEffect(() => {
    if (classInfo === null) setSelfStudyStatus(() => undefined);
  }, [classInfo]);
  ...
  useEffect(() => {
    if (!myData) return;
    if (isStudent(myData)) setHasUserClassInfo(() => !!myData.class);
    ...
    <DndProvider backend={HTML5Backend}>
      ...
      >
        <TopBar
          ...
          canSelectOtherClass={
            hasUserClassInfo === undefined ? undefined : !hasUserClassInfo
          }
          clasName={
            classInfo ? `${classInfo[0]}학년 ${classInfo[1]}반` : undefined
          }
          selfStudyName={
            currentSelfStudyTime
              ? {
        ...
        <TableWrapper>
          <div>
            {classInfo !== null ? (
              [
                {
                  color: selfStudyStatus
                    ? ROW_COLOR.AVAILABLE
                    : ROW_COLOR.NOTAVAILABLE,
                  label: '현원',
                ...
                >
                  ...
                  >
                    {(
                      type.places ||
                      (IS_MOVING_CLASS_SYSTEM
                        ? groupedPlacesIncludingMovingClass
                        : groupedPlaces)
                      ...
                      return (
                        <React.Fragment key={place.name}>
                          {placeIndex !== 0 && (
                            <Divider data-divider horizontal smaller />
                          )}
                          <ResponsiveWrapper threshold={800} key={place.name}>
                            <ResponsiveWrapper
                              css={css`
                                min-height: 67px;
                              `}
                            ...
                            >
                              ...
                              <LabelCard
                                title="인원"
                                hasLabel={hasLabel}
                                css={responsiveLabelCardWidth(75)}
                              >
                                {isRealData(place) ? (
                            ...
                            <StudentList
                              ...
                              rowType={place.type}
                              log={
                                isRealData(place) ? place.students : undefined
                              }
                              moveStudent={
                                place &&
                                ((student) =>
                                  moveStudentPlaceTo(student, place))
                              }
                              isDraggable={myData?.permissions.includes(
                ...
                {[...Array(3)].map((_, gradeIndex) => (
                  <GridRow key={`grade${gradeIndex}`}>
                    {[...Array(6)].map((__, classIndex) => (
                      ...
                      >
                        <CardHeader>
                          {gradeIndex + 1}학년 {classIndex + 1}반
                        </CardHeader>
                      </ClassCard>
                    ))}
                  </GridRow>
              ...
              {classInfo && (
                <>
                  <LabelCard title="총원" hasLabel width={70}>
                    {studentQuantity ? (
                      studentQuantity.available + studentQuantity.notAvailable
                    ) : (
                      <Skeleton width={30} />
                    )}
                  </LabelCard>
                  ...
                  <LabelCard
                    ...
                    width={70}
                    css={css`
                      --row-color: ${ROW_COLOR.NOTAVAILABLE};
                    `}
                  >
                    {studentQuantity ? (
                      studentQuantity.notAvailable
                    ) : (
                      <Skeleton width={30} />
                    )}
                  </LabelCard>
