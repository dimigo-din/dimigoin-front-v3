...
const sortByLogAnd = (log: AttendanceLogWithStudent[]) => {
    const [hasLog, noLog] = log.sort((a, b) => a.student.number - b.student.number).reduce<AttendanceLogWithStudent[][]>((grouped, current) => {
                        ...
                        >
                            {student.student.number.toString().padStart(2, '0')} {student.student.name}
                            {
                                ((content: string) => content && <EtcInfo>{content}</EtcInfo>)([
                                    ...
                                    student.log?.remark && ['CIRCLE', 'ETC'].includes(rowType) && student.log.remark
                                ].filter(Boolean).join(', '))
                            }
                        </DraggableStudent>
