...
} from '../Afterschool/AfterschoolEditor';
...
const toInputTime = (time: HourAndMinute) =>
...
export const MentoringEditor: React.FC<{
  ...
  const [dayChecks, setDayChecks] = useState<boolean[]>(
    [...Array(5)].map(
      (_, index) => !!data?.days.includes(Object.keys(EngDay)[index] as EngDay),
    ),
  );
  const gradeInput = useInput<RadioButtonItem>(
    data
      ? {
  ...
  useEffect(() => {
    setDayChecks(() =>
      [...Array(5)].map(
        (_, index) =>
          !!data?.days.includes(Object.keys(EngDay)[index] as EngDay),
      ),
    );
  }, [data]);
  ...
  const [startTimeInput] = useTextInput(
    data ? toInputTime(data.duration.start) : undefined,
  );
  const [endTimeInput] = useTextInput(
    data ? toInputTime(data.duration.end) : undefined,
  );
  ...
  const register = useCallback(() => {
    const checks = [
      ...
      getCheckedIndex(dayChecks.map((checked) => ({ checked }))).length === 0 &&
        '요일',
    ...
    const newData: RequestableMentoring = {
      days: getCheckedIndex(dayChecks.map((checked) => ({ checked }))).map(
        (index) => engDays[index] as EngDay,
      ),
      duration: {
    ...
    (data
      ...
      .then((e) => {
        toast.success(
          data ? '멘터링 정보를 수정했어요' : '새 멘토링을 등록했어요',
        );
      })
      .catch((e) => {
        toast.error(
    ...
    <>
      ...
      <Horizontal>
        {[...Array(5)].map((_, index) => (
          <Checkbox
            key={`day${index}`}
            defaultChecked={data?.days.includes(
              Object.keys(EngDay)[index] as EngDay,
            )}
            checked={dayChecks[index]}
            onChange={({ target: { checked } }) =>
              setDayChecks((beforeState) => [
      ...
      <Horizontal>
        <Input type="time" {...startTimeInput} /> <Separator>~</Separator>
        <Input type="time" {...endTimeInput} />
      </Horizontal>
      <RegisterButtonWrapper>
