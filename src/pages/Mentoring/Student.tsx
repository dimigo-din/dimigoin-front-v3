...
import {
  ...
  applyMentoring as _applyMentoring,
  unApplyMentoring as _unApplyMentoring,
  getAppliedMentoring,
} from '../../api/mentoring';
import {
  Button,
...
const Student: React.FC = () => {
  const [mentoringSchedules, setMentoringSchedules] = useState<
    Doc<MentoringSchedule>[] | null
  >();
  const [appliedMentoring, setAppliedMentoring] = useState<
    Doc<MentoringApplication>[] | null
  >();

  const fetchData = useCallback(() => {
    getRequestableMentoringList()
      .then(setMentoringSchedules)
      .catch(() => setMentoringSchedules(null));
    getAppliedMentoring()
  ...
  return (
    <>
      <PageWrapper>
        <CardGroupHeader>신청하기</CardGroupHeader>
        <Table
          css={css`
            width: 100%;
          `}
        >
          ...
          {mentoringSchedules ? (
            mentoringSchedules.length ? (
              mentoringSchedules
                .map((mentoringSchedule) => ({
                  ...mentoringSchedule,
                  application: appliedMentoring
                    ? appliedMentoring.find(
                        (application) =>
                ...
                .map(
                  (
                    mentoringSchedule: Doc<MentoringSchedule> & {
                      application?: Doc<MentoringApplication>;
                    },
                  ) => (
