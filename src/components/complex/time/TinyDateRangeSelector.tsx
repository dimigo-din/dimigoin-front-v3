...
export const useTinyDateRangeSelector = (): {
  ...
  const toggleOpened = useCallback((e: { target: EventTarget }) => {
    if (yPosition) {
  ...
  const applyChangedDate = useCallback(selectedDates => {
    setDates(() => selectedDates)
  }, [setDates])

  const openA18yInput = useCallback((e: { target: EventTarget, key: string }) => {
  ...
  return {
    dates,
    element: <span>
      <InputForAccessability onKeyPress={openA18yInput} aria-label="게시일을 선택해주세요" />
